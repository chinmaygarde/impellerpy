...
def test_paragraph_builder_with_multiple_languages():
    ...
    jp_style.set_font_foreground(
        Paint().set_color(Color(r=0.8, g=0.0, b=0.0, a=1.0))
    )
    ...
    ar_style.set_font_foreground(
        Paint().set_color(Color(r=0.0, g=0.0, b=0.8, a=1.0))
    )
    ...
    assert (
        paragraph.line_count() >= 3
    )  # At least 3 lines (one for each language)
